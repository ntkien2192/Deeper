...
//  Futurep
//
//  Created by Nguyễn Trung Kiên on 10/4/19.
...
public struct Json {
    ...
    public init(_ object: Any) {
        ...
        case let object as Data:
            do { try self.init(data: object)
            } catch { self.init(JsonObject: NSNull()) }
...
extension Json: Swift.RawRepresentable {
    ...
    public var rawValue: Any { return object }
...
extension Json {
    public var array: [Json]? { return type == .array ? rawArray.map { Json($0) } : nil }
    public var arrayValue: [Json] { return self.array ?? [] }
...
extension Json {
    static func get(file name: String) -> Json? {
        if let path = Bundle.main.path(forResource: name, ofType: "json") {
            do {
                let data = try Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe)
                let json = try Json(data: data, options: .mutableLeaves)
                return json
            } catch {
                return nil
            }
